...
type Action =
  ...
  | {
      type: "SET_FILES";
      payload: { tagId: string; files: FileDataWithHandle[] };
    }
  | { type: "CLEAR_FILES" }
  | { type: "RESET_STATE" }
...
export default function TemplateDisplay({
  ...
  const [templateHandle, setTemplateHandle] =
  ...
  const parseTemplate = (content: string): Segment[] => {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      const segmentType =
        tagType === "superpromptor-file" ? "fileSelector" : "input";
  ...
  const createFileSelectionHandler = useCallback(
    (tagId: string) => (files: FileDataWithHandle[]) =>
      handleFilesSelected(tagId, files),
    [handleFilesSelected],
  );
  ...
  const handleRefresh = async () => {
    try {
            ...
            files.map(async (fileData) => {
              if (fileData.handle) {
                try {
                  ...
                  console.error(
                    `Error refreshing file ${fileData.path}:`,
                    error,
                  );
      ...
      setAlertMessage(
        "Failed to refresh. Some files may have been moved or deleted.",
      );
      setAlertType("error");
    }
  };
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      setAlertMessage(
        "Failed to copy to clipboard. Please ensure clipboard permissions are enabled.",
      );
      setAlertType("error");
    }
  };
      ...
      if (segment.type === "markdown") {
        ...
        return (
          <div key={segment.id} className="block">
            <FileSelector id={segment.id} onFilesSelected={onFilesSelected} />
          </div>
        );
      } else if (segment.type === "input") {
            ...
            <Textarea
              ...
              onChange={(e) =>
                dispatch({
                  ...
                  payload: { inputId: segment.id, value: e.target.value },
                })
              }
    ...
    <div className="p-4">
      {state.segments.length > 0 ? (
        <div>
          ...
          <div className="flex flex-col gap-2">{renderTemplate()}</div>
          <div className="mt-4">
            <button
              onClick={handleCopy}
